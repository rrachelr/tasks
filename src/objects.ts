...
export function isValid(question: Question, answer: string): boolean {
    const options = question.options.reduce(
        (total: string, option: string) => total + option,
        ""
    );
    const result =
        question.type === "multiple_choice_question" &&
        options.includes(answer) === true
            ? true
            : false;
    return question.type === "short_answer_question" || result;
}
...
export function toShortForm(question: Question): string {
    ...
    return question.id.toString() + ": " + first10;
}
...
export function toMarkdown(question: Question): string {
    const options = question.options.reduce(
        (total: string, option: string) =>
            question.options.indexOf(option) === 2
                ? total + "- " + option
                : total + "- " + option + "\n",
        ""
    );
